...
import com.example.merchantapp.nfcutils.CommandApdu;
import com.example.merchantapp.nfcutils.ResponseUtils;
import com.example.merchantapp.nfcutils.TlvUtil;
import com.example.merchantapp.nfcutils.TrackUtils;
...
public class EmvParser {
	 ...
	 * Provider
	 */
	private io.github.tapcard.emvnfccard.parser.IProvider provider;
	 ...
	 */
	public EmvParser(final io.github.tapcard.emvnfccard.parser.IProvider pProvider, final boolean pContactLess) {
		provider = pProvider;
	...
	protected List<byte[]> getAids(final byte[] pData) {
		...
		for (TLV tlv : listTlv) {
			if (tlv.getTag() == EmvTags.KERNEL_IDENTIFIER && ret.size() != 0) {
				ret.add( com.example.merchantapp.emvn.ArrayUtils.addAll(ret.get(ret.size() - 1), tlv.getValueBytes()));
			} else {
	 ...
	 */
	protected boolean parse(final byte[] pSelectResponse, final io.github.tapcard.emvnfccard.parser.IProvider pProvider) throws CommunicationException {
		boolean ret = false;
		// Get TLV log entry
	...
	protected boolean extractCommonsCardData(final byte[] pGpo) throws CommunicationException {
		...
		if (data != null) {
			data = com.example.merchantapp.emvn.ArrayUtils.subarray(data, 2, data.length);
	...
	protected void extractCardHolderName(final byte[] pData) {
		...
		if (cardHolderByte != null) {
			String[] name = com.example.merchantapp.nfcmodel.StringUtils.split(new String(cardHolderByte).trim(), CARD_HOLDER_NAME_SEPARATOR);
			if (name != null && name.length == 2) {
				card.setHolderFirstname( com.example.merchantapp.nfcmodel.StringUtils.trimToNull(name[0]));
				card.setHolderLastname( com.example.merchantapp.nfcmodel.StringUtils.trimToNull(name[1]));
			}
		}
	}
	 ...
	 * @return return data
	 */
	protected byte[] getGetProcessingOptions(final byte[] pPdol, final io.github.tapcard.emvnfccard.parser.IProvider pProvider) throws CommunicationException {
