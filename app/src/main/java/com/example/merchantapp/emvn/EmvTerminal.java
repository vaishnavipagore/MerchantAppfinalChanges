...
public final class EmvTerminal {
	...
	public static byte[] constructValue(final TagAndLength pTagAndLength) {
		...
		if (pTagAndLength.getTag() == EmvTags.TERMINAL_TRANSACTION_QUALIFIERS) {
			...
			val = BytesUtils.fromString(com.example.merchantapp.emvn.StringUtils.leftPad(String.valueOf(CountryCodeEnum.FR.getNumeric()), pTagAndLength.getLength() * 2,
					"0"));
		} else if (pTagAndLength.getTag() == EmvTags.TRANSACTION_CURRENCY_CODE) {
			val = BytesUtils.fromString(com.example.merchantapp.emvn.StringUtils.leftPad(String.valueOf(CurrencyEnum.EUR.getISOCodeNumeric()),
